...
def dibujarenemigos(amount, posx, posf, xi, xf, yf):

    global enemigos
    if amount == 1:
        posx = xi + 6
        posf = xf + 6
        if posx - enemigos[-1][0] > 6:
...
def actualizar_posicion(xi,xf, yi,yf, ranxi, ranxf, posx, posf, vidas):
    ...
    while True:
        ...
        if yi != 0:
            timeintheair += 1
        if timeintheair > 3:
            if yf == 3.2 and yi == 2.2:          
                yi = 0
                yf = 1
            elif yf == -3.2 and yi == -2.2:
                yi = 0
                yf = -1
            jumpping = False
            timeintheair = 0
        ...
        if yf > 0:
            c = 1
        dibujarenemigos(amount,posx, posf, xi, xf, c)
        print("amount", amount, "posx", posx, "posf", posf, "xi", xi, "xf", xf, "yf", yf, "c", c)
        contador += 1
        if contador == 10:
            if yf < 0:
                yf = 1
