...
import main, maintwo, mainthree, mainfour, mainfive, mainsix
...
def play_music():
    pygame.mixer.init()
    pygame.mixer.music.load("stay.mp3")  # Ruta a tu archivo de música
...
def verificar_respuesta(valor, correcto, nivel):
    ...
    if variables.pregunta == 4 and variables.vidas == 0:
       while True:
            mb.showwarning("cochino", "Cochino hp!")
        
    elif variables.pregunta ==  4 and variables.vidas > 0:
        preguntas_window.destroy()
        if nivel == 1:
            main.main(variables.vidas)
        elif nivel == 2:
            maintwo.main(variables.vidas)
        elif nivel == 3:
            mainthree.main(variables.vidas)
        elif nivel == 4:
            mainfour.main(variables.vidas)
        elif nivel == 5:
            mainfive.main(variables.vidas)
        elif nivel == 6:
            mainsix.main(variables.vidas)
...
#--------------------------------------------boton enviar-----------------------------------------
                ...
                buton_enviar = ctk.CTkButton(master = frame_scroll_preguntas,
                                            ...
                                            fg_color = "white",
                                            hover_color = "light blue",
                                            command = lambda: verificar_respuesta(radiobar2.get(), "a", valor)
                                            )
                buton_enviar.pack(padx=5, pady=2, expand=True, anchor="n")
#--------------------------------------------pregunta 3-----------------------------------------
            elif pregunta == 3:
