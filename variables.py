...
La función raíz cuadrada está dada por la ecuación f(x)= √x, y solo tiene sentido para los valores de x que 
cumplan con la condición, ya que en el conjunto de los números reales las raíces de índice par con radicando 
negativo, no están definidas. Su expresión analítica es algebraica irracional, su curva es la mitad de una 
parábola.
...
"""
raiz_cuadrada_2 = """A este gráfico se pueden aplicar traslaciones horizontales, hacia la derecha si hacemos 
x − 1 y hacia la izquierda si hacemos x + 1. También se pueden aplicar traslaciones verticales, se desplaza 
hacia arriba si se suman constantes positivas, mientras que se desplaza hacia abajo si se suman constantes 
negativas.

"""
raiz_cuadrada_3 = """Si se multiplica la función por una constante negativa, la función raíz cuadrada se refleja 
con respecto al eje 𝑥.

"""
exponencial_1 = """FUNCIÓN EXPONENCIAL

Las funciones exponenciales son aquellas que tienen la variable independiente x en el exponente de una potencia. 
...
Propiedades
	...
	El dominio de una función exponencial son todos los números reales o, dicho con otras palabras, una función 
    exponencial existe por cualquier valor de x.

	La función solo toma valores positivos, por lo tanto, el rango de una función exponencial son todos los números 
    reales positivos.

	Toda función exponencial es una función continua e inyectiva a la vez.

	Si la función no está trasladada, cualquier función exponencial pasa por el punto (0,1). Porque la función 
    evaluada en el cero siempre da como resultado uno.

	Asimismo, el valor de una función exponencial en x=1 es igual a la base.

	La inversa de la función exponencial es la función logarítmica. Por tanto, las gráficas de una función exponencial 
    y una función logarítmica son simétricas respecto de la recta y=x si ambas poseen la misma base.

Gráfica

"""
exponencial_2 = """La función por la derecha sigue creciendo hasta el infinito, en cambio, la función por la izquierda 
va decreciendo, pero nunca llega a 0. Aunque se acerca mucho, nunca lo llega a tocar. Eso quiere decir que la recta y=0 
(el eje de las abscisas) es una asíntota horizontal.
"""

logaritmica_1 = """FUNCIÓN LOGARÍTMICA
Una función logarítmica es aquella que se expresa como log_a⁡x, siendo a la base de esta función, que ha de ser positiva y 
distinta de 1.
...
Así, se tiene que:
	...
	La función logarítmica sólo existe para valores de x positivos, sin incluir el cero. Por tanto, su dominio son los 
    reales positivos mayores que 0.
...
La función logarítmica, puede ser cambiada en k unidades verticalmente y h unidades horizontalmente con la ecuación 
y = log b ( x + h ) + k .
...
La función seno de un ángulo α es una función trigonométrica cuya fórmula se define como la razón entre el cateto opuesto
y la hipotenusa de un triángulo rectángulo (triángulo con un ángulo recto).
...
El signo de la función seno depende del cuadrante en el que se encuentre el ángulo: si el ángulo está dentro del primero 
o segundo cuadrantes el seno será positivo, por contra, si el ángulo cae en el tercero o cuarto cuadrante el seno será 
negativo.
...
"""
seno_3 = """Como se puede ver en la gráfica, los valores de las imágenes de la función seno siempre están entre +1 y -1, 
es decir, está acotada superiormente por +1 e inferiormente por -1. Además, los valores se van repitiendo cada 360 grados 
(2π radianes), por lo que se trata de una función periódica cuyo periodo es 360º.

Propiedades de la función seno

La función seno tiene las siguientes características:

•	El dominio de la función seno son todos los números reales ya que, como se ve en la gráfica, la función existe por 
cualquier valor de la variable independiente x.

•	El recorrido o rango de la función seno va desde -1 hasta 1 (ambos incluidos).

•	Este tipo de función trigonométrica tiene un único punto de corte con el eje de las ordenadas (eje Y) en el 
punto (0,0).

Periodo y amplitud

La función seno es una función periódica, es decir, sus valores se van repitiendo según una frecuencia. Además, los 
valores máximos y mínimos entre los que oscila depende de su amplitud. Por lo tanto, dos rasgos que determinan la 
función seno son su periodo y su amplitud: 
